...
export default function CreateProfile() {
  ...
  // =======================================================
  // NAYA JADOO: EVENT LISTENER (Live Notification)
  // =======================================================
  useEffect(() => {
    if (!wallet) return;
    const { program } = getProgramAndPda();

    // Hum program ko bol rahe hain ke "profileCreatedEvent" ka intezar karo
    const listenerId = program.addEventListener("profileCreatedEvent", (event, slot) => {
      console.log("LIVE EVENT RECEIVED:", event);
      alert(`🔔 LIVE NOTIFICATION 🔔\n\n${event.message}\nUser ka Naam: ${event.name}\nWallet Address: ${event.authority.toBase58()}`);
    });

    // Cleanup (Jab component band ho toh listener hata do)
    return () => {
  ...
  const handleCreateProfile = async () => {
    ...
    try {
      ...
      await program.methods.createProfile(name).accounts({
          userProfile: profilePda,
          user: publicKey,
          systemProgram: anchor.web3.SystemProgram.programId,
        }).rpc(); 
      // Yahan humne manually alert hata diya hai, kyunke ab alert EVENT ke zariye aayega!
    } catch (error: any) { 
      console.error(error);
      alert(error.message || "Koi masla aa gaya!"); 
    }
  };
  ...
  const handleDeleteProfile = async () => {
    if (!publicKey || !wallet) return alert("Pehle Wallet Connect Karo!");
    const confirmDelete = window.confirm("Kya tum waqai apni profile delete karna chahte ho?");
